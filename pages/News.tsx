...
import { Calendar, Search, ArrowRight, Filter, ChevronLeft, ChevronRight, Tag } from 'lucide-react';
...
const News: React.FC = () => {
  ...
  const categories = ['Semua', ...Array.from(new Set(MOCK_NEWS.map(item => item.category)))];

  useEffect(() => {
    // Dynamic Fetch Simulation from an API endpoint
    const fetchNewsData = async () => {
      ...
      // In a real app, these params would be sent to the API
      // const response = await fetch(`/api/news?page=${currentPage}&category=${selectedCategory}&search=${searchTerm}`);
      ...
      // Pagination Logic (Server-side simulation)
  ...
  // Skeleton Loader Component mimicking the News Card
  const NewsSkeleton = () => (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-12">
            {loading ? (
               [...Array(ITEMS_PER_PAGE)].map((_, i) => <NewsSkeleton key={i} />)
                ...
                news.map(item => (
                <Link to={`/berita/${item.id}`} key={item.id} className="bg-white dark:bg-slate-800 rounded-2xl shadow-lg border border-gray-100 dark:border-gray-700 hover:shadow-2xl transition-all duration-300 overflow-hidden group flex flex-col h-full animate-fade-in hover:-translate-y-1">
