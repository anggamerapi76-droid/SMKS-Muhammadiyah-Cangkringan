...
import { Major, NewsItem, SiteSettings } from '../types';
import { Calendar, ArrowRight, Award, CheckCircle2, Target, Zap, Shield, Wifi, PenTool, Monitor, Quote, Book } from 'lucide-react';
...
const Home: React.FC = () => {
  ...
  return (
    <div className="bg-gray-50 dark:bg-slate-900">
      <SEO 
        title="Beranda" 
        description={`${settings.schoolName} - ${settings.tagline}`} 
      />
              ...
              <div className="lg:w-2/3 text-white">
                ...
                <div className="mt-8 pt-8 border-t border-white/10 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                   <div>
                      ...
                      <p className="text-muca-yellow text-sm">Kepala {settings.shortName}</p>
                   </div>
          ...
          <div className="flex flex-col lg:flex-row items-center gap-12">
            ...
            <div className="lg:w-1/2">
              ...
              <p className="text-gray-600 dark:text-gray-300 text-lg mb-8 leading-relaxed">
                Sebagai SMK Pusat Keunggulan (SMK PK), {settings.schoolName} dipercaya oleh Kemendikbudristek untuk menjadi sekolah rujukan dan pusat peningkatan kualitas dan kinerja. Kami menerapkan kurikulum yang diselaraskan dengan dunia kerja.
              </p>
           ...
           <div className="flex flex-col md:flex-row gap-12 items-center">
              ...
              <div className="md:w-2/3 grid grid-cols-2 gap-4">
                  {settings.facilities.map((facility) => {
                    const IconComponent = iconMap[facility.icon] || Shield;
                    return (
                      <div key={facility.id} className="bg-slate-50 dark:bg-slate-700 p-6 rounded-xl text-center hover:shadow-lg transition-shadow border border-gray-100 dark:border-gray-600">
                         <IconComponent className="mx-auto text-muca-yellow mb-3" size={32} />
        ...
        <div className="container mx-auto px-4 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {news.slice(0, 3).map(newsItem => (
              <div key={newsItem.id} className="bg-white dark:bg-slate-800 rounded-2xl shadow-lg border border-gray-100 dark:border-gray-700 hover:shadow-2xl transition-all duration-300 overflow-hidden group">
                <div className="relative h-56 overflow-hidden">
                  <div className="absolute inset-0 bg-slate-900/20 group-hover:bg-slate-900/10 transition-colors z-10"></div>
                  <img src={newsItem.image} alt={newsItem.title} loading="lazy" className="w-full h-full object-cover transform group-hover:scale-110 transition-transform duration-700" />
                ...
                <div className="p-8">
                  ...
                  <p className="text-gray-500 dark:text-gray-400 mb-6 line-clamp-2 leading-relaxed">{newsItem.excerpt}</p>
                  <Link to={`/berita/${newsItem.id}`} className="inline-flex items-center text-slate-900 dark:text-white font-bold hover:text-muca-yellow transition-colors">
